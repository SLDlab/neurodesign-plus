...
from neurodesign_legacy import generate_legacy as generate, report_legacy as report
...
class Design:
    ...
    def FdCalc(self, Aoptimality=True):
        ...
        if Aoptimality is True:
            # np.matrix.trace() on an np.matrix returns a 1x1 matrix (ndim=2),
            # which current NumPy refuses to implicitly convert with float().
            # .item() extracts the scalar without changing the computed value.
            self.Fd = float(len(self.C) / np.asarray(np.matrix.trace(CMC)).item())
        else:
